...
async def get_performance(
    ...
    trades = outcome_tracker.get_trades_log(
        limit=limit,
        timeframe=timeframe,
        dual_ai_only=dual_ai_only,
        session=session,
...
async def export_performance_history(
    ...
    session: Optional[str] = Query(None, description="Optional session filter (e.g. London, Overlap, Asian)"),
    category: Optional[str] = Query(None, description="Optional category filter (e.g. Forex, Crypto, Indices)"),
    symbol: Optional[str] = Query(None, description="Optional symbol filter (e.g. BTC, NDX)")
):
    ...
    sess_tag = f"_{session}" if session and session.upper() != "ALL" else ""
    cat_tag = f"_{category}" if category and category.upper() != "ALL" else ""
    ...
    if format.lower() == "csv":
        ...
        headers = {
            "Content-Disposition": f'attachment; filename="trade_history{tf_tag}{sess_tag}{cat_tag}{dual_tag}_{now_str}.csv"'
        }
        ...
        headers = {
            "Content-Disposition": f'attachment; filename="trade_history{tf_tag}{sess_tag}{cat_tag}{dual_tag}_{now_str}.json"'
        }
